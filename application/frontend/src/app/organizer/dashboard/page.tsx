  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [eventsResponse, statsResponse] = await Promise.all([
        ...
        api.get('/orders/stats/overview').catch(() => ({ data: { totalOrders: 0, totalRevenue: 0, totalTickets: 0 } }))
      ]);
      ...
      setStats({
        totalEvents: eventsResponse.data.events?.length || eventsResponse.data?.length || 0,
        totalRevenue: statsData.totalRevenue || 0,
        totalTicketsSold: statsData.totalTickets || 0,
        averageTicketPrice: statsData.totalTickets > 0 ? (statsData.totalRevenue / statsData.totalTickets) : 0,
        upcomingEvents: eventsResponse.data.events?.filter((event: Event) => new Date(event.startAt) > new Date()).length || 0,
        recentOrders: []
      });
    } catch (error) {
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-3 flex items-center space-x-4 text-sm">
                          <span className="text-gray-600">
                            Revenue: <span className="font-medium text-green-600">${formatPrice(getTotalRevenue(event))}</span>
                          </span>
                          <span className="text-gray-600">
                            Sold: <span className="font-medium">{getTotalTicketsSold(event)}</span>
                          </span>
                        </div>
                      </div>
          ...
          <div>
            ...
            {stats?.recentOrders && stats.recentOrders.length === 0 ? (
              <Card className="p-6 text-center">
                ...
                <p className="text-gray-500">Orders will appear here once customers start purchasing tickets</p>
              </Card>
            ) : (
              <div className="space-y-4">
