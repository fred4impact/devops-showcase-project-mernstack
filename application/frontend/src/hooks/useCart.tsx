...
interface CartContextType {
  ...
  loading: boolean
  sessionId: string
  addToCart: (ticketTypeId: string, quantity: number, seatId?: string) => Promise<void>
  updateCartItem: (ticketTypeId: string, quantity: number, seatId?: string) => Promise<void>
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToCart = async (ticketTypeId: string, quantity: number, seatId?: string) => {
    try {
      setLoading(true)
  ...
  const updateCartItem = async (ticketTypeId: string, quantity: number, seatId?: string) => {
    try {
      setLoading(true)
      const response = await cartApi.updateCartItem(sessionId, ticketTypeId, { quantity }, seatId)
      setCart(response.data)
  ...
  const removeFromCart = async (ticketTypeId: string, seatId?: string) => {
    try {
      setLoading(true)
      const response = await cartApi.removeFromCart(sessionId, ticketTypeId, seatId)
      setCart(response.data)
