...
export const eventsApi = {
  ...
  getMyEvents: (params?: { page?: number; limit?: number }) =>
    api.get<{ events: Event[]; pagination: any }>('/events/my-events', {
      params,
    }),
...
export const cartApi = {
  getCart: (sessionId: string) => api.get<Cart>(`/cart?sessionId=${sessionId}`),
  ...
  validateCart: (sessionId: string) =>
    api.get<{ isValid: boolean; errors: string[] }>(
      `/cart/validate?sessionId=${sessionId}`,
    ),
};
...
export const ordersApi = {
  createOrder: (data: any, sessionId?: string) =>
    api.post<Order>(
      `/orders${sessionId ? `?sessionId=${sessionId}` : ''}`,
      data,
    ),
  ...
  getMyOrders: (params?: { page?: number; limit?: number }) =>
    api.get<{ orders: Order[]; pagination: any }>('/orders/my-orders', {
      params,
    }),
  ...
  getOrdersByEmail: (
    ...
    api.get<{ orders: Order[]; pagination: any }>(
      `/orders/by-email?email=${email}`,
      { params },
    ),

  cancelOrder: (id: string) => api.put<Order>(`/orders/${id}/cancel`),

  getEventStats: (eventId: string) => api.get(`/orders/stats/event/${eventId}`),
